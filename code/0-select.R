...
df_thrust <- cbind(advance_speed, penet_rate, thrust_force, thrust_stroke) %>%
  replace_zero_df(.)
...
earth_press <- data %>%
  ...
  set_df(.) %>%
  set_colnames(., c('ChamberSoilPresUp', 
                    'ChamberSoilPres', 
                    'ChamberSoilPresLow'))

earth_press['ChamberSoilDensity'] <- (earth_press['ChamberSoilPresLow'] - earth_press['ChamberSoilPresUp']) * 100 / 17

df_chamber <- earth_press#['ChamberBulkDensity']

# earth_press_ave <- data %>%
#   # select(contains('earth')) %>%
#   select(contains('e-pres')) %>%
#   .[ , which(.[1,] == 'MPa')] %>%
#   set_df(.) %>%
#   rowMeans(., na.rm = F) %>% as_tibble(.) %>%
#   set_colnames(., 'EarthPressAve')
...
screw_press <- data %>%
  ...
  .[ , which(.[1,] == 'bar')] %>%
  set_df(.) %>%
  set_colnames(., c('ScrewPress1A',
                    'ScrewPress1B',
                    'ScrewPress2A',
                    'ScrewPress2B'))

# screw_press['ScrewPress'] <- rowSums(screw_press, na.rm = T)
...
screw_rot['ScrewRotSpeed'] <- rowMeans(screw_rot, na.rm = T)
...
df_screw <- cbind(screw_press, screw_torque, screw_rot,
...
df_grout <- cbind(grout_vol, grout_press) #, grout_flow, grout_fill_rate)
...
foam_liquid_press <- data %>%
  select(contains('foam liquid')) %>%
  select(contains('press')) %>%
  .[ , which(.[1,] == 'bar')] %>%
  set_df(.) %>%
  rowSums(., na.rm = T) %>% as_tibble(.) %>%
  set_colnames(., 'FoamLiquidPress')
...
water_vol <- data %>%
  select(contains('water supply vol')) %>%
  .[ ,1:2] %>%
  set_df(.) %>%
  rowSums(., na.rm = T) %>% as_tibble(.) %>%
  set_colnames(., 'WaterVol')
...
df_foam <- cbind(foam_press, foam_vol, foam_flow, #foam_fill_rate,
                 foam_liquid_press, foam_liquid_vol, foam_liquid_flow,
                 water_vol,
                 foam_agent_vol, foam_agent_flow,
                 air_press, air_vol, air_flow)
...
df_polymer <- cbind(polymer_press, polymer_vol, polymer_flow,
...
# clean df
df <- df %>% 
  # replace values higher/lower than Q +/- (10 * IQR) with NA
  replace_outliers_df(.) #%>%
  # replace zero and negative values with NA
  # replace_negative_df(.) 
...
# df <- df %>% 
  ...
  # select(!contains('Flow')) %>%
  # select(!contains('FillRate')) #%>%
  # select(!contains('EP')) %>%
  # select(!contains(c('1', '2'))) #%>%
  # select(!contains('AppDensity')) #%>%
  # select(!contains('ScrewMuck')) %>%
  # select(!contains(c('lower', 'Upper'))) %>%
  # select(!contains('Rear'))
...
plot(x = df$Chainage, y = (df$FoamLiquidVol - df$FoamAgentVol), type = 'l')
lines(x = df$Chainage, y = df$WaterVol, col = 'red')
lines(x = df$Chainage, y = df$FoamLiquidVol, col = 'blue')
